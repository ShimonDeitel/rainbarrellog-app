...
struct ContentView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Rain Barrel Log")
            .toolbar {
...
struct EntryFormView: View {
    ...
    @State private var fillLevel: String = ""
    ...
    init(mode: EntryFormMode) {
        ...
        if case .edit(let entry) = mode {
            ...
            _fillLevel = State(initialValue: entry.fillLevel)
    ...
    var body: some View {
        NavigationStack {
            Form {
                DatePicker("Last Cleaned / Checked", selection: $date, displayedComponents: .date)
                    .accessibilityIdentifier("entryDateField")
                TextField("Fill Level", text: $fillLevel)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    ...
                    .disabled(fillLevel.trimmingCharacters(in: .whitespaces).isEmpty)
                }
            }
        }
    }
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = CheckEntry(date: date, fillLevel: fillLevel, notes: notes)
            store.add(entry)
        case .edit(let existing):
            var updated = existing
            updated.date = date
            updated.fillLevel = fillLevel
            updated.notes = notes
